...
describe('TruthTable', () => {
  ...
  test('rejects truth tables large enough to freeze the browser', () => {
    const letters = Array.from({ length: MAX_TRUTH_TABLE_LETTERS + 1 }, (_, index) => letter({ id: 'A', index: index + 1 }))
    expect(() => make_tt(letters)).toThrow(`At most ${MAX_TRUTH_TABLE_LETTERS} distinct sentence letters are supported`)
  })
})
