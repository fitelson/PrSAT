...
## Eliminate Ratios Option (branch `prsat-3.1-eliminate-ratios`)

Optional solver preprocessing (checkbox next to "Regular:"): rewrites every atomic comparison into `N op 0` with no division anywhere — denominators cleared by cross-multiplication — before sending to Z3. `N` stays in compact factored form; products are NOT expanded into monomials (full expansion was tried first and crashed the browser: AST depth ~1200 overflowed the recursive MathML renderer, and SMT-LIB hit ~50KB vs ~2KB structural on likelihood-ratio constraints). Implemented in `src/eliminate_ratios.ts` as symbolic rational-function arithmetic over `RealExpr`, sharing common denominator factors. Soundness: denominators built from state-variable sums and nonneg literals are ≥ 0 by the probability axioms and ≠ 0 by the existing div-0 guards, so they drop from inequalities; unknown-sign denominators (free real variables) get a sign case-split; `=`/`≠` ignore denominator signs (guards). Runs after `enrich_constraints` (guards derived from the division form) and before `eliminate_state_variable_index`. Option threaded as `eliminate_ratios` through `pr_sat_wrapped` / `pr_sat_with_options`. The translated-constraints display keeps the ratio form — the transformation only changes the solver input.
